<template>
  <div class="bg-primary  articles scroller">
    <div class="card mb-3 bg-transparent p-0">
      ...
      <div class="row px-0 mt-4" v-for="(item, $index) in listOne" :key="$index">
      ...
      <client-only>
        <infinite-loading @infinite="infiniteHandlerOne" v-if="listOne.length !== 0"></infinite-loading>
      </client-only>
    </div>
...
export default {
  ...
  methods: {
    infiniteHandlerOne($state){
      this.$axios
        .get('/user/posts?limit=5&count=5&type=savings&lang=' + this.$i18n.locale , {
          headers: {
        ...
        .then((response) => {
          ...
          if (Feed.length) {
            this.page += 1
            this.listOne.push(...response.data.results)
            $state.loaded()
          } else {
            $state.complete()
          }
    ...
    sortFunction(command) {
      if (command === 'b') {
        ...
        return this.listOne.sort(function (a, b) {
          if (a.posts) {
      ...
      if (command === 'e') {
        this.shuffleArray(this.listOne)
      }
    },
